...
class MenuRenderer:
    ...
    def render_file_menu(epd, files, selected_index, max_width=400, max_height=300):
        """
        Render file selection menu on display

        Args:
            epd: Display object
            files: List of filenames
            selected_index: Currently selected file index
            max_width: Display width (default 400)
            max_height: Display height (default 300)
        """
        ...
        title_height = CHAR_HEIGHT + 5
        footer_height = CHAR_HEIGHT * 2
        available_height = max_height - title_height - footer_height
        max_visible_files = available_height // CHAR_HEIGHT

        # Calculate scroll window
        if len(files) <= max_visible_files:
            # All files fit on screen
            window_start = 0
            window_end = len(files)
        else:
            # Need scrolling - keep selected item in view
            window_start = max(0, selected_index - max_visible_files // 2)
            window_end = min(len(files), window_start + max_visible_files)
            # Adjust if at end of list
            if window_end - window_start < max_visible_files:
                window_start = max(0, window_end - max_visible_files)

        # Draw title
        epd.image1Gray.text("Select File:", MARGIN_LEFT, MARGIN_TOP, epd.black)

        # Draw files
        y = MARGIN_TOP + title_height
        for i in range(window_start, window_end):
            filename = files[i]
            # Truncate filename if too long (fit within display width)
            max_chars = (max_width - MARGIN_LEFT - 20) // CHAR_WIDTH
            ...
            epd.image1Gray.text(text, MARGIN_LEFT, y, epd.black)
            y += CHAR_HEIGHT

        # Draw footer with instructions
        footer_y = max_height - footer_height
        epd.image1Gray.text(f"File {selected_index + 1}/{len(files)}",
                           MARGIN_LEFT, footer_y, epd.black)
        epd.image1Gray.text("Up/Down=Nav Enter=Open N=New Esc=Exit",
                           MARGIN_LEFT, footer_y + CHAR_HEIGHT, epd.black)

    @staticmethod
    def render_prompt(epd, prompt, current_value="", max_width=400, max_height=300):
        """
        Render text input prompt on display
        ...
        # Draw prompt
        epd.image1Gray.text(prompt, MARGIN_LEFT, MARGIN_TOP, epd.black)

        # Draw current input value
        input_y = MARGIN_TOP + CHAR_HEIGHT * 2
        epd.image1Gray.text("Input: " + current_value, MARGIN_LEFT, input_y, epd.black)

        # Draw cursor indicator (underline at end of input)
        cursor_x = MARGIN_LEFT + (len("Input: ") + len(current_value)) * CHAR_WIDTH
        epd.image1Gray.fill_rect(cursor_x, input_y + CHAR_HEIGHT - 2,
                                 CHAR_WIDTH, 2, epd.black)

        # Draw instructions
        footer_y = max_height - CHAR_HEIGHT * 2
        epd.image1Gray.text("Enter=Confirm  Esc=Cancel",
                           MARGIN_LEFT, footer_y, epd.black)
...
Application Startup
    |
    v
show_menu() --> List .txt files from STORAGE_BASE
    |
    v
MenuRenderer.render_file_menu(epd, files, selected_index)
    |
    v
User Navigation (Up/Down arrows, Enter to select, N to create new, Esc to exit)
    |
    v
File Selected --> Load file into editor
    |
    v
Editor Mode (typing, saving, etc.)
"""
