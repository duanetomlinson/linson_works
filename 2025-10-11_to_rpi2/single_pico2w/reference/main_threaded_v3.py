"""
main_threaded_v3.py - Dual-core threading with full key combo support
Version 3: Adds complete key combination functionality

CHANGES IN V3:
==============
1. Added Ctrl+S/O/N/R/D/T key combos (save, open, new, rename, delete, todoist)
2. Added Alt+Backspace for word deletion
3. Added Shift+Enter for explicit page markers (---)
4. Added PgUp/PgDn/Home for page navigation (read-only view mode)
5. Added in_paged_view mode and page navigation state
6. Added all action functions (action_save, action_open, action_new, etc.)
7. Added helper functions (delete_word, new_page_marker, display_page, etc.)

FIXES FROM V2:
==============
✓ SPI lock for thread-safe display operations
✓ Display re-initialization before partial/full refreshes
✓ Clean menu-to-editor transitions
✓ GC on Core 0 only (thread-safe)
