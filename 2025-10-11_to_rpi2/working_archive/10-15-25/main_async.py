...
async def main_async():
    ...
    ACTIVE_FILE = f"{STORAGE_BASE}/async_test.txt"
    try:
    ...
    epd.image1Gray.text("Async mode active", 10, 30, epd.black)
    epd.image1Gray.text("Tasks:", 10, 50, epd.black)
    epd.image1Gray.text("- Keyboard (10ms)", 10, 70, epd.black)
    epd.image1Gray.text("- Display (500ms)", 10, 90, epd.black)
    epd.image1Gray.text("- File save (2s)", 10, 110, epd.black)
    await refresh_full_async(epd)
