...
# Updated with async display updates for smooth writing
...
# Import our custom queue implementation
...
# Async display settings
MIN_UPDATE_INTERVAL = 200      # Minimum ms between updates
KEY_BUFFER_SIZE = 10          # Keys to buffer before update
TYPING_MODE_TIMEOUT = 2000    # Ms of inactivity to exit typing mode
FULL_REFRESH_INTERVAL = 20    # Partial refreshes before full
...
# Key buffer for async processing
key_buffer = []
last_render_time = 0
...
class SmartDisplayManager:
    """Intelligent display update management for e-ink"""
    
    def __init__(self):
        self.last_full_refresh = utime.ticks_ms()
        self.partial_count = 0
        self.max_partials = FULL_REFRESH_INTERVAL
        self.typing_mode = False
        self.last_keypress = 0
    
    def request_update(self, force_full=False):
        """Smart update decision making"""
        now = utime.ticks_ms()
        
        # Force full refresh periodically to prevent ghosting
        if force_full or self.partial_count >= self.max_partials:
            request_display_update('full', priority=2)
            self.partial_count = 0
            self.last_full_refresh = now
        else:
            # Use partial refresh during typing
            request_display_update('partial', priority=1)
            self.partial_count += 1
    
    def enter_typing_mode(self):
        """Optimize for continuous typing"""
        self.typing_mode = True
        self.last_keypress = utime.ticks_ms()
    
    def check_typing_mode(self):
        """Exit typing mode after inactivity"""
        if self.typing_mode:
            if utime.ticks_diff(utime.ticks_ms(), self.last_keypress) > TYPING_MODE_TIMEOUT:
                self.typing_mode = False
                # Do a clean full refresh after typing stops
                self.request_update(force_full=True)
...
def request_display_update(update_type='partial', priority=0, region=None):
    ...
    try:
        ...
        log_exception(e, "request_display_update")
...
def render_buffer_async():
    """Render current state to buffer without blocking"""
    # This runs in main thread but doesn't do actual display update
    with display_lock:
        # Render current text
...
def refresh_display_async():
    """Request async display refresh"""
    ...
    # Request update
    display_manager.request_update()
    display_dirty = False

# Replace existing refresh_display with async version
refresh_display = refresh_display_async
...
# No-render versions for buffered processing
def insert_char_no_render(ch: str):
    """Insert character without refreshing display"""
...
def backspace_no_render():
    """Delete character without refreshing display"""
    global text_buffer, cursor_index, file_dirty
...
def cursor_newline_no_render():
    """Insert newline without refreshing display"""
    insert_char_no_render('\n')

def delete_word_no_render():
    """Delete word before cursor without refresh"""
    global text_buffer, cursor_index, file_dirty
...
def process_single_key(key_data):
    """Process a single key without rendering"""
    lbl, shift_on, alt_on, ctrl_on = key_data
    
    # Your existing key processing logic but WITHOUT refresh_display calls
    if lbl == 'Backspace':
        if alt_on:
            delete_word_no_render()
        else:
            backspace_no_render()
    elif lbl == 'Enter':
        if shift_on:
            ...
            cursor_newline_no_render()
    elif lbl == 'Space':
        insert_char_no_render(' ')
    elif len(lbl) == 1:
        ch = glyph(lbl, shift_on)
        insert_char_no_render(ch)

def flush_key_buffer():
    """Process all buffered keys and update display"""
    global key_buffer, last_render_time
    
    if not key_buffer:
        return
    
    # Process all buffered keys
    for key_data in key_buffer:
        process_single_key(key_data)
    ...
    render_buffer_async()
    display_manager.request_update()
    last_render_time = utime.ticks_ms()

# Original versions with display refresh (keep for immediate operations)
def check_overflow():
            ...
            for k in new_keys:
                ...
                if lbl in ['PgUp', 'Up'] and idx > 0:
                    ...
                    if action_delete(to_remove):
                        ...
                        draw_menu()  # Redraw to remove any dialog
...
def action_delete(path: str = None):
    """
    ...
    while True:
        ...
        if new_keys:
            for k in new_keys:
                ...
                # Buffer normal typing keys
                if lbl and not ctrl_on:
                    # Buffer the key instead of processing immediately
                    key_buffer.append((lbl, shift_on, alt_on, ctrl_on))
                    keys_processed += 1
                    display_manager.enter_typing_mode()
            
            last_key_time = now
        
        prev_keys = pressed
        
        # Flush key buffer periodically or when idle
        if keys_processed > 0:
            if keys_processed >= KEY_BUFFER_SIZE or utime.ticks_diff(now, last_key_time) > 50:
                flush_key_buffer()
                keys_processed = 0
        
        # Check typing mode
        display_manager.check_typing_mode()
        
        # Update display periodically if dirty
        if display_dirty and utime.ticks_diff(now, last_display_update) > MIN_UPDATE_INTERVAL:
        ...
        if utime.ticks_diff(now, last_display_update) > 5000:
            gc.collect()
        
        time.sleep_ms(5)  # Shorter sleep for better responsiveness
    
    # Cleanup on exit (shouldn't normally reach here)
    stop_display_thread()
