...
# For ESP32-S3-WROOM-1 with full pin support and improved punctuation handling
# 
# COMPLETE WIRING GUIDE:
#   TCA8418 VCC → ESP32-S3 3.3V
#   TCA8418 GND → ESP32-S3 GND
#   TCA8418 SDA → ESP32-S3 GPIO 4  (I2C Data)
#   TCA8418 SCL → ESP32-S3 GPIO 5  (I2C Clock) 
#   TCA8418 INT → ESP32-S3 GPIO 21 (Interrupt - RECOMMENDED for efficiency)
#   TCA8418 RST → ESP32-S3 GPIO 38 (Reset - OPTIONAL for recovery)
#
...
class TCA8418:
    ...
    def __init__(self, i2c, addr=None, interrupt_pin=None, reset_pin=None):
        ...
        if self.interrupt_pin:
            ...
            print(f"INT pin configured on GPIO {self.interrupt_pin}")
        
        # Configure reset pin if provided
        if self.reset_pin:
            self.reset_pin.init(Pin.OUT, value=1)  # Start high (not in reset)
            print(f"RST pin configured on GPIO {self.reset_pin}")
...
# Linson Keyboard Implementation - Main Program
def linson_keyboard_main():
    """Main keyboard loop for Linson keyboard with key names"""
    
    # Initialize I2C with clean pins for ESP32-S3-WROOM-1
    i2c = I2C(0, scl=Pin(5), sda=Pin(4), freq=400000)
    
    # Configure interrupt and reset pins (RECOMMENDED)
    interrupt_pin = Pin(21, Pin.IN, Pin.PULL_UP)  # INT pin - active low
    reset_pin = Pin(38, Pin.OUT, value=1)         # RST pin - active low
    
    try:
        # Initialize keyboard controller with interrupt and reset support
        keyboard = TCA8418(i2c, interrupt_pin=interrupt_pin, reset_pin=reset_pin)
        print("=" * 60)
        print("    LINSON KEYBOARD READY WITH INTERRUPT SUPPORT!")
        print(f"    INT pin: GPIO 21, RST pin: GPIO 38")
        print("    Press keys to see them detected")
        print("    Ctrl+C to exit")
        print("=" * 60)
        
        # Track currently pressed keys for proper press/release detection
        currently_pressed = set()
        
        while True:
            # Check for key events using interrupt pin (much more efficient!)
            ...
            utime.sleep_ms(1)  # Very small delay - interrupt-driven is efficient!
            
    except OSError as e:
        print(f"ERROR: {e}")
        print("Check wiring:")
        print("  TCA8418 VCC  → ESP32-S3 3.3V")
        print("  TCA8418 GND  → ESP32-S3 GND")
        print("  TCA8418 SDA  → ESP32-S3 GPIO 4")
        print("  TCA8418 SCL  → ESP32-S3 GPIO 5")
        print("  TCA8418 INT  → ESP32-S3 GPIO 21 (RECOMMENDED)")
        print("  TCA8418 RST  → ESP32-S3 GPIO 38 (OPTIONAL)")
    except KeyboardInterrupt:
        print("\nLinson keyboard monitoring stopped.")


# Basic test function for debugging
def test_tca8418():
    """Test function for TCA8418 with GPIO 4/5 and optional INT/RST pins"""
    
    # Initialize I2C with GPIO 4/5
    i2c = I2C(0, scl=Pin(5), sda=Pin(4), freq=400000)
    
    # Optional: Configure interrupt and reset pins
    interrupt_pin = Pin(21, Pin.IN, Pin.PULL_UP)  # INT pin - recommended
    reset_pin = Pin(38, Pin.OUT, value=1)         # RST pin - optional
    ...
    try:
        ...
        if 0x34 in devices:
            print("✓ TCA8418 found at 0x34!")
            
            # Initialize TCA8418 with interrupt and reset support
            keypad = TCA8418(i2c, interrupt_pin=interrupt_pin, reset_pin=reset_pin)
            print("✓ TCA8418 initialized successfully with INT/RST support!")
            
            # Main loop - show both row/col and key names
            last_keys = []
            print("\nPress keys on your keyboard (Ctrl+C to exit)...")
            print("Using interrupt-driven detection for maximum efficiency!")
            
            while True:
                # Check for key events using interrupt pin
                if keypad.has_interrupt():
                    # Get raw positions
                    pressed_keys = keypad.scan_keys()
                    
                    if pressed_keys != last_keys:
                        if pressed_keys:
                            # Show both raw positions and key names
                            for row, col in pressed_keys:
                                key_name = keypad.get_key_name(row, col)
                                print(f"Key at ({row},{col}): {key_name}")
                        else:
                            print("All keys released")
                        last_keys = pressed_keys[:]
                
                utime.sleep_ms(1)  # Very responsive with interrupt support
        else:
            print("✗ TCA8418 not found!")
            print("Check wiring:")
            print("  VCC → 3.3V")
            print("  GND → GND")
            print("  SDA → GPIO 4")
            print("  SCL → GPIO 5")
            print("  INT → GPIO 21 (recommended)")
            print("  RST → GPIO 38 (optional)")
...
def test_connection():
    """Quick test to verify TCA8418 connection"""
    i2c = I2C(0, scl=Pin(5), sda=Pin(4), freq=400000)
    
    print("Testing TCA8418 connection...")
    print(f"Using pins: SCL=GPIO5, SDA=GPIO4")
    
    devices = i2c.scan()
    print(f"I2C devices found: {[hex(d) for d in devices]}")
    
    if 0x34 in devices:
        print("✓ SUCCESS: TCA8418 found at address 0x34")
        print("\nRecommended additional connections:")
        print("  INT → GPIO 21 (for efficient interrupt-driven operation)")
        print("  RST → GPIO 38 (for hardware reset capability)")
        return True
    else:
        print("✗ FAILED: TCA8418 not found")
